...
## Aktuální stav — image workflow a inventura 2026-09-12

- Aktivní program obsahuje přesně 51 unikátních ID; `swan` je mimo program, `dumbbell_pullover` je aktivní.
- Obrazové SOURCE: 49 nové prostředí, 1 rozpracovaný/mapping k ověření (`sideplank_reach`), 1 staré prostředí k opravě (`swimming`), 0 prokázaně chybějících aktivních bundle.
- Muscle Card k doladění: 9. Guide/Step k aktualizaci: 2 cviky / 4 karty.
...
- `sideplank_reach`: fyzicky existují unversioned START/HERO/END, ale runtime stále odkazuje na staré neexistující `*_v01.png`; final approval a Guide/Step obsah je nutné ověřit. Jde o mapping bug k samostatnému úkolu, ne o pokyn generovat nový SOURCE.
- NEXT TASK podle obrazové fronty: samostatně ověřit a opravit mapping `sideplank_reach`; následně převést `swimming` ze starého prostředí a aktualizovat jeho Guide/Step. Muscle Card OPEN úkoly se řídí přesnými profily v `MASTER_ANATOMY.md`.
- Dumbbell Pullover (`dumbbell_pullover`) používá schválené bezverzové START/HERO, END = START; Guide/Step v01 jsou hotové. Muscle Card zůstává OPEN podle `MASTER_ANATOMY.md`. Chest Opener a Russian mají rovněž Muscle Card OPEN, protože jejich dřívější shared PNG už v repozitáři nejsou; nedoložená runtime mapování byla odstraněna. Russian (`russian`) používá bezverzové START/HERO, END = START a znovu sestavené Guide/Step v01. Chest Opener (`chest_opener`) zůstává správně napojený na bezverzové SOURCE a Guide/Step v04. Schválená Muscle Card `glutes_primary_hamstrings_core_secondary_muscles_v01.png` přesně odpovídá profilům `hip`, `donkey` a `glute_bridge_march` (PRIMARY A11; SECONDARY A14 + A01). `frog` ji v runtime nyní používá chybně, protože jeho autoritativní profil je A11 + A01; oprava mapování je samostatný implementační úkol. Schválená `quadriceps_primary_hamstrings_secondary_muscles_v01.png` zůstává bez runtime přiřazení, protože žádný aktivní profil přesně neodpovídá A13 + A14. Program, dávky, pořadí, detailový layout a workout logika zůstaly beze změny. Aktuální runtime verze/cache: `v59.192-dev`; `style.css?v=59187anatomyfit`; `data.js?v=59206sourcebatch`; `app.js?v=59207masterconsolidation`.
...
- The Hundred a Bird Dog používají schválený core anatomy reuse z Heel Taps. Pro Thread the Needle neexistuje schválená mapa horních zad/hrudní páteře, proto nebyla použita anatomicky nepřesná náhrada. Verze/cache: `v59.170-dev`; `app.js?v=59181sourcebatch2`; `data.js?v=59181sourcebatch2`; `style.css?v=59179masterdetailfinal`.
- Dne 2026-09-08 byly Hamstring Stretch (`hamstring_supine`), Hip March (`hip_march`), Reverse Crunch (`revcrunch`) a Scissors (`scissors`) lokálně přepojeny na finální bezverzové SOURCE. Existující Guide/Step exporty zachovaly rozměry a strukturu; změnily se pouze vložené fotografie. Scissors používá START → HERO → MID a runtime střídání HERO ↔ MID; END všech čtyř cviků používá START.
- Hip March, Reverse Crunch a Scissors používají schválený core anatomy reuse z Heel Taps. Pro Hamstring Stretch neexistuje ve verzované knihovně schválená hamstring mapa, proto nebyla použita nesprávná hýžďová mapa ani vytvořena náhrada.
- Glute Bridge March Guide Card byla opravena na 3 panely START → MID → HERO. Step by Step zůstal beze změny a runtime `gluteBridgeMarchFlow` zůstává START → MID → HERO → MID → START.
- Společný detailový renderer používá schválené pořadí HERO → miniatury → DETAIL CVIKU/název → DECH → ZAPOJENÉ SVALY | NA CO SI DÁT POZOR → Krok za krokem, včetně dvou sloupců při 320 px. Knihovní náhledy dokončených SOURCE balíků ukazují na stejné finální HERO.
...
- Glute Bridge nyní používá přesný reuse Hollow Hold START v05 a approved HERO v03 (`aeaaa7d2…`) se sekvencí START → HERO → START. Existující Guide v3 a Step v2 byly pouze přerenderovány s novými fotografiemi. Sdílená svalová karta `Muscle_Cards/glutes_muscles_v01.png` zvýrazňuje hýžďové svaly podle Moovka MASTER BODY standardu.
- Mapování aplikace `hollow` a jeho náhled byly cíleně aktualizované na finální SOURCE; mapování `hip` a Glute Bridge anatomy zůstaly beze změny. Cache parametry jsou `app.js?v=59164hollowfinal` a `data.js?v=59165hollowfinal`. Program, dávky, workout, ostatní cviky a PWA nebyly změněny.

- Standing Side Bend / `standing_side_bend` je dokončený s approved START v03, HERO v03, HERO opposite v01 a anatomy v01. Guide Card v01 (780 × 1688 RGB) a Step by Step v01 (780 × 2280 RGB) byly reprodukované a vizuálně ověřené v sekvenci START → HERO 1 → START → HERO 2 → START; END je přesně START bez nového PNG. Sdílená anatomy `Muscle_Cards/obliques_muscles_v01.png` je napojená podle oblasti šikmých břišních svalů. SOURCE ani anatomy PNG nebyly upravované.
...
- Hollow Hold používá finální sekvenci `hollow_hold_start.png` → `hollow_hold_hero.png` → `hollow_hold_start.png`; Guide Card a Step by Step v01 jsou aktualizované a cvik je nasazený v aplikaci pod ID `hollow`.
...
- `Pilates Assets/02_Exercise_Cards/Hollow Hold/hollow_hold_start.png`
