...
START = BASE / "roll_up_start_v01.png"
HERO = BASE / "roll_up_hero_v01.png"
END = BASE / "roll_up_end_v01.png"
GUIDE = BASE / "roll_up_guide_card_v01.png"
STEP = BASE / "roll_up_step_by_step_v01.png"
EXPECTED_HASHES = {
    START: "d75bb4378aea8d9a6727abf0e370ff6505e71cb0d7b4c4177e289186f930d72f",
    HERO: "1adc44b14d64fdafb8b12f156a16b79841637e1379e9e31b32ce1c858b5d9c87",
    END: "1e32bb5e872dff64d548005e9809f5f82dd4c50a7ba2e557dcb0725f7f573eaf",
}
...
GUIDE_HOW = [
    ("1", "Lehni si na záda, nohy natáhni a spoj. Paže veď za hlavu."),
    ("2", "S výdechem zvedej hlavu, ramena a páteř obratel po obratli. Paže veď vpřed."),
    ("3", "Dokonči pohyb v sedu a poté se s nádechem pomalu vrať do výchozí polohy."),
]

GUIDE_WATCH = (
    "Nezvedej nohy z podložky a nepoužívej švih. Ramena drž daleko od uší "
    "a páteř roluj postupně."
)
...
STEP_TEXTS = [
    (
        "KROK 1",
        "VÝCHOZÍ LEH",
        "Lehni si na záda. Nohy natáhni a drž těsně vedle sebe. Paže veď za hlavu a střed těla jemně zpevni.",
        START,
    ),
    (
        "KROK 2",
        "ROLOVÁNÍ VZHŮRU",
        "S výdechem zvedej hlavu, ramena a páteř obratel po obratli. Paže veď plynule vpřed a nohy nech na podložce.",
        HERO,
    ),
    (
        "KROK 3",
        "SED A NÁVRAT",
        "Dokonči pohyb v kontrolovaném sedu s pažemi vpřed. Poté se pomalu obratel po obratli vrať do výchozí polohy.",
        END,
    ),
]

STEP_BREATH = "Výdech při rolování vzhůru. Nádech při návratu."
STEP_WATCH = (
    "Nepoužívej švih a nezvedej nohy z podložky. Ramena nestahuj k uším "
    "a pohyb veď plynule ze středu těla."
)
...
def build_guide():
    ...
    draw.text((62, 49), "ROLL UP", font=F["title"], fill=INK)
    draw.text((62, 94), "Střed těla a kontrola páteře", font=F["small_b"], fill=TEAL_D)
    _, desc_bottom = draw_wrapped(
        draw, (62, 115), "Roluj páteř obratel po obratli do sedu a kontrolovaně zpět.",
        F["tiny"], MUTED, 650, 2
    )
    ...
    xs = [34, 274, 514]
    labels = [
        ("START", "Leh na zádech", START),
        ("ROLOVÁNÍ", "Paže vpřed", HERO),
        ("SED", "Kontrolované dokončení", END),
    ]
    ...
    info = [
        ("breath", "DECH", "Výdech vzhůru. Nádech při návratu."),
        ("focus", "ZAMĚŘ SE", "Páteř roluj postupně. Nohy drž dole."),
        ("repeat", "OPAKOVÁNÍ", "6×"),
    ]
...
def build_step():
    ...
    draw.text((62, 98), "ROLL UP", font=F["small_b"], fill=TEAL_D)
