...
START = BASE / "leg_raises_start_v02.png"
HERO = BASE / "leg_raises_hero_v01.png"
GUIDE = BASE / "leg_raises_guide_card_v01.png"
STEP = BASE / "leg_raises_step_by_step_v01.png"
EXPECTED_HASHES = {
    START: "0c63347400015112353a0f481f5dbb57c7538fbe8b937de8bea65a27b2eaffc7",
    HERO: "d55c1fda81a2239b0983db4303961f0eaa6c4d112be542d97a78ea2a666127a8",
}
...
GUIDE_HOW = [
    ("1", "Lehni si na záda, nohy natáhni a zpevni střed těla. Bedra drž na podložce."),
    ("2", "Pomalu zvedej obě natažené nohy vzhůru. Pánev zůstává stabilní."),
    ("3", "Spouštěj nohy zpět jen tak nízko, dokud bedra zůstávají na podložce."),
]

GUIDE_WATCH = (
    "Bedra drž stabilně na podložce a nohy zvedej bez švihu. "
    "Pohyb zastav dříve, než se bedra začnou odlepovat."
)
...
STEP_TEXTS = [
    (
        "KROK 1",
        "VÝCHOZÍ POZICE",
        "Lehni si na záda. Nohy drž natažené a zpevni střed těla. Bedra udržuj stabilně na podložce.",
        START,
    ),
    (
        "KROK 2",
        "ZVEDNUTÍ NOHOU",
        "Pomalu zvedej obě natažené nohy vzhůru kontrolovaným pohybem. Pánev zůstává stabilní.",
        HERO,
    ),
    (
        "KROK 3",
        "KONTROLOVANÉ SPOUŠTĚNÍ",
        "Pomalu spouštěj natažené nohy zpět do výchozí polohy. Zastav pohyb dříve, než by se začala odlepovat bedra od podložky.",
        START,
    ),
]

STEP_BREATH = "Vydechuj při zvednutí. Nadechuj se při spouštění."
STEP_WATCH = "Bedra drž na podložce. Nohy zvedej bez švihu."
...
def build_guide():
    ...
    draw.text((62, 49), "ZVEDÁNÍ NOHOU", font=F["title"], fill=INK)
    draw.text((62, 94), "Spodní břicho a stabilita trupu", font=F["small_b"], fill=TEAL_D)
    _, desc_bottom = draw_wrapped(
        draw, (62, 115), "Posiluje spodní břicho a učí stabilní polohu beder.",
        F["tiny"], MUTED, 650, 2
    )
    margins["GUIDE DESCRIPTION"] = 140 - desc_bottom
    x = pill(draw, (62, 148), "Spodní břicho", F["small_b"])
    ...
    info = [
        ("breath", "DECH", "Výdech při zvednutí. Nádech při spouštění."),
        ("focus", "ZAMĚŘ SE", "Bedra drž na podložce a pánev stabilní."),
...
def build_step():
    ...
    draw.text((62, 98), "ZVEDÁNÍ NOHOU", font=F["small_b"], fill=TEAL_D)
