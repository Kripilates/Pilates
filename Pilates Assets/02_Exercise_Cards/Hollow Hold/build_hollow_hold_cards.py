...
START = BASE / "hollow_hold_start.png"
HERO = BASE / "hollow_hold_hero.png"
...
EXPECTED_FILE_HASHES = {
    ...
    HERO: "afeb196fd1a616069fba6b165e8e52614c95074821828ccd020ded86faeaa140",
}
EXPECTED_HERO_PIXEL_HASH = "0dfec21b86d88634a72c03fc72a87d9652b613eda45ec9a074dbb60d9881946c"
BG = (244, 251, 250)
CARD = (255, 255, 255)
INK = (38, 44, 47)
MUTED = (101, 115, 119)
...
def verify_sources():
    ...
    if pixel_sha256(HERO) != EXPECTED_HERO_PIXEL_HASH:
        raise RuntimeError(f"Pixel SHA mismatch for {HERO.name}")
...
def build_guide():
    ...
    info = [
        ...
        ("focus", "ZAMĚŘ SE", "Bedra drž pevně na podložce."),
...
def build_step():
    ...
    _, watch_bottom = draw_wrapped(draw, (62, next_y + 54), STEP_WATCH, F["step_body"], INK, 650, 3)
...
def main():
    verify_sources()
    before = {path: sha256(path) for path in EXPECTED_FILE_HASHES}
    guide_margins = build_guide()
    step_margins = build_step()
    after = {path: sha256(path) for path in EXPECTED_FILE_HASHES}
    if before != after:
