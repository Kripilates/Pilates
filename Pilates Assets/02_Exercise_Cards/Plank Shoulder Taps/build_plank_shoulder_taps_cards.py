...
START = BASE / "plank_shoulder_taps_start_v02.png"
HERO = BASE / "plank_shoulder_taps_hero_v01.png"
GUIDE = BASE / "plank_shoulder_taps_guide_card_v01.png"
STEP = BASE / "plank_shoulder_taps_step_by_step_v01.png"
EXPECTED_HASHES = {
    START: "da60e08494d12e271bd3a42d90216b63d9ba3ca83de7d79f07fa3bf33d361f27",
    HERO: "83695e392cab0560ed18bbcdde6ec9607e6b14224bb50ccb434a1f68a8ed13ad",
}
...
GUIDE_HOW = [
    ("1", "Zaujmi vysoký plank. Dlaně dej pod ramena a chodidla mírně šíře než boky."),
    ("2", "Zpevni střed a s výdechem se jednou rukou dotkni opačného ramene."),
    ("3", "Vrať dlaň pod rameno a plynule vystřídej strany bez pohybu pánve."),
]

GUIDE_WATCH = (
    "Pánev drž stabilní a boky neotáčej. Nepropadej se v bedrech ani mezi rameny, "
    "hlavu nech v prodloužení páteře."
)
...
STEP_TEXTS = [
    (
        "KROK 1",
        "VYSOKÝ PLANK",
        "Zaujmi vysoký plank. Dlaně polož pod ramena a chodidla dej mírně šíře než boky. Tělo drž v jedné dlouhé linii.",
        START,
    ),
    (
        "KROK 2",
        "DOTYK RAMENE",
        "Zpevni břicho a s výdechem zvedni jednu ruku. Kontrolovaně se dotkni opačného ramene a pánev drž klidnou.",
        HERO,
    ),
    (
        "KROK 3",
        "VYSTŘÍDÁNÍ STRAN",
        "Vrať dlaň pod rameno a proveď pohyb druhou rukou. Boky drž stále co nejvíce bez pohybu.",
        START,
    ),
]

STEP_BREATH = "Výdech při dotyku. Nádech při návratu."
STEP_WATCH = (
    "Pánev drž stabilní. Nepropadej se v bedrech ani mezi rameny "
    "a pohyb veď bez švihu."
)
...
def build_guide():
    ...
    draw.text((62, 49), "DOTYKY RAMEN V PRKNĚ", font=F["title"], fill=INK)
    draw.text((62, 94), "Střed těla a ramena", font=F["small_b"], fill=TEAL_D)
    _, desc_bottom = draw_wrapped(
        draw, (62, 115), "Posiluje střed těla a učí držet pánev stabilní při střídání rukou.",
        F["tiny"], MUTED, 650, 2
    )
    ...
    xs = [34, 274, 514]
    labels = [
        ("START", "Vysoký plank", START),
        ("DOTYK", "Opačné rameno", HERO),
        ("NÁVRAT", "Dlaň pod rameno", START),
    ]
    ...
    info = [
        ("breath", "DECH", "Výdech při dotyku. Nádech při návratu."),
        ("focus", "ZAMĚŘ SE", "Stabilní pánev a pevný střed těla."),
...
def build_step():
    ...
    draw.text((62, 98), "DOTYKY RAMEN V PRKNĚ", font=F["small_b"], fill=TEAL_D)
