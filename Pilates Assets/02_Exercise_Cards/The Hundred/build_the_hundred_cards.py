...
def build_guide():
    ...
    draw.text((62, 48), "THE HUNDRED", font=F["title"], fill=INK)
    draw.text((62, 98), "Posílení středu těla s dechem", font=F["subtitle"], fill=TEAL_D)
    x = pill(draw, (62, 154), "Střed těla", F["small_b"])
    x = pill(draw, (x, 154), "Bez pomůcky", F["small_b"])
    pill(draw, (x, 154), "Časový cvik", F["small_b"])
    ...
    xs = [34, 274, 514]
    labels = [
        ("START", "Nohy v 90/90", START),
        ("HERO", "Zvedni lopatky", HERO),
    ...
    info = [
        ...
        ("Zaměř se", ["Střed těla pevný.", "Krk bez tlaku."], "focus"),
        ("Opakování", ["Lehká/Střední 20-25 s", "Náročná 35 s"], "repeat"),
    ]
    ...
    guide_lines = [
        "1. Lehni si na záda a zvedni nohy do 90/90.",
        "2. Lehce zpevni břicho a prodluž krk.",
        "3. Zvedni hlavu a lopatky do pracovní pozice.",
        "4. Pažemi prováděj malé kontrolované pulzy.",
    ]
    y = 1230
    ...
    draw_wrapped(
        draw,
        (62, 1544),
        "Netlač bradu k hrudníku, nezvedej ramena k uším a drž bedra stabilní. Paže kmitají jen v malém rozsahu.",
        F["body"],
        MUTED,
        650,
        7,
...
def build_step():
    ...
    draw.text((62, 94), "The Hundred", font=F["small_b"], fill=TEAL_D)

    steps = [
        ("START", START, "Lehni si na záda. Nohy zvedni do 90/90, paže nech dlouhé podél těla a krk drž uvolněný."),
        ("PRACOVNÍ POZICE", HERO, "Zpevni střed těla a zvedni hlavu s lopatkami. Paže drž nízko nad podložkou."),
        ("MALÉ PULZY", HERO, "V pracovní pozici prováděj malé kontrolované pulzy natažených paží. Dýchej plynule a nezadržuj dech."),
        ("NÁVRAT", START, "Po dokončení polož hlavu a lopatky zpět. Nohy vrať kontrolovaně bez prohnutí v bedrech."),
    ]
    ...
    draw_wrapped(
        draw,
        (62, 2186),
        "Dýchej plynule po celou dobu. Udrž střed těla aktivní a nepřetěžuj krk.",
        F["body"],
        MUTED,
        650,
        7,
