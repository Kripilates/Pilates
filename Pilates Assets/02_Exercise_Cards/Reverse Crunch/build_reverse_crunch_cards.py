...
GUIDE_HOW=[
('1','Lehni si na záda, paže nech podél těla a nohy zvedni do tabletop.'),
('2','S výdechem podsadíš pánev a jemně přitáhneš kolena směrem k hrudníku.'),
('3','S nádechem vrať pánev kontrolovaně zpět a nohy do výchozí pozice.'),
]
GUIDE_WATCH='Nešvihej nohama, nezvedej ramena k uším a nepřenášej pohyb do krku. Bedra pokládej kontrolovaně.'
STEP_TEXTS=[
('KROK 1','VÝCHOZÍ POZICE',START,'Lehni si na záda, paže nech podél těla a nohy zvedni do tabletop. Hlava a ramena zůstávají na podložce.'),
('KROK 2','PODSADIT PÁNEV',HERO,'S výdechem veď pohyb z břicha, jemně podsadíš pánev a přitáhneš kolena směrem k hrudníku.'),
('KROK 3','KONTROLOVANÝ NÁVRAT',START,'S nádechem vrať pánev dolů bez švihu a nastav nohy zpět do tabletop.'),
]
STEP_WATCH='Nešvihej nohama, nezvedej ramena k uším a nepřenášej pohyb do krku. Bedra pokládej kontrolovaně.'
...
F={'title':font('arialbd.ttf',42),'h2':font('arialbd.ttf',25),'h3':font('arialbd.ttf',20),'body':font('arial.ttf',22),'small':font('arial.ttf',17),'small_b':font('arialbd.ttf',17),'tiny':font('arial.ttf',14),'step_title':font('arialbd.ttf',38),'step_h':font('arialbd.ttf',24),'step_body':font('arial.ttf',23)}
...
def center_text(d,box,text,fnt,fill):
    x1,y1,x2,y2=box; b=d.textbbox((0,0),text,font=fnt)
...
def pill(d,xy,text,fnt,fill=SOFT,outline=LINE,txt=TEAL_D):
    x,y=xy; b=d.textbbox((0,0),text,font=fnt); px,py=16,8
...
def draw_icon(d,center,kind,color=TEAL):
    x,y=center
    if kind=='breath':
        d.arc((x-8,y-8,x+8,y+8),25,320,fill=color,width=3); d.line((x+7,y-8,x+12,y-8,x+12,y-3),fill=color,width=3)
    elif kind=='focus':
        d.ellipse((x-8,y-8,x+8,y+8),outline=color,width=3); d.ellipse((x-3,y-3,x+3,y+3),fill=color)
    elif kind=='repeat':
        d.arc((x-9,y-7,x+9,y+7),200,20,fill=color,width=3); d.polygon([(x+8,y-8),(x+14,y-5),(x+9,y-1)],fill=color)
    elif kind=='warn':
        d.polygon([(x,y-10),(x-10,y+9),(x+10,y+9)],outline=color,width=3); d.line((x,y-3,x,y+3),fill=color,width=2); d.ellipse((x-1,y+6,x+1,y+8),fill=color)

def build_guide():
    img=Image.new('RGB',(780,1688),BG); d=ImageDraw.Draw(img)
    rounded(d,(34,34,746,132),28,CARD,LINE,2)
    d.text((62,52),'ZKRACOVAČKA OBRÁCENĚ',font=F['title'],fill=INK)
    d.text((62,98),'REVERSE CRUNCH',font=F['small_b'],fill=TEAL_D)
    draw_wrapped(d,(62,118),'Posiluje spodní část břicha a učí kontrolované podsazení pánve.',F['tiny'],MUTED,650,2)
    x=62; x=pill(d,(x,142),'Břicho',F['small_b']); pill(d,(x,142),'Kontrola pánve',F['small_b'])
    rounded(d,(34,196,746,653),28,CARD,LINE,2); paste_round(img,fit_image(HERO,(680,393)),(50,218,730,611),22)
    mini_y,mini_w,mini_h,gap=675,218,146,22; xs=[34,34+mini_w+gap,34+2*(mini_w+gap)]
    labels=[('START','Nohy v tabletop'),('PODSADIT','Kolena k hrudníku'),('NÁVRAT','Zpět do startu')]
    ...
    info=[('breath','DECH','Výdech při podsazení pánve. Nádech při návratu.'),('focus','ZAMĚŘ SE','Pohyb veď z břicha, ne švihem nohou.'),('repeat','OPAKOVÁNÍ','Plynule podle dávky v tréninku.')]
    ...
    d.text((62,1120),'JAK PROVÉST',font=F['h2'],fill=INK); y=1165
...
def build_step():
    img=Image.new('RGB',(780,2280),BG); d=ImageDraw.Draw(img)
    rounded(d,(34,34,746,126),28,CARD,LINE,2)
    d.text((62,56),'Krok za krokem',font=F['step_title'],fill=INK)
    d.text((62,98),'REVERSE CRUNCH / ZKRACOVAČKA OBRÁCENĚ',font=F['small_b'],fill=TEAL_D)
    y=160
    for step_label,heading,photo,body in STEP_TEXTS:
        ...
        draw_wrapped(d,(62,y+486),body,F['step_body'],INK,650,7)
        y += card_h+18
...
def main():
    start_hash=sha256(START); hero_hash=sha256(HERO)
    build_guide(); build_step()
