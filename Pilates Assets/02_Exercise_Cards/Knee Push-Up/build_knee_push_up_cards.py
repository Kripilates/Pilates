...
START = BASE / "knee_push_up_start_v01.png"
HERO = BASE / "knee_push_up_hero_v01.png"
GUIDE = BASE / "knee_push_up_guide_card_v01.png"
STEP = BASE / "knee_push_up_step_by_step_v01.png"
EXPECTED_HASHES = {
    START: "3e393647e0840da10536cef4cb5f970b726081fcb8a169a450d92663e22ca87a",
    HERO: "661b25c58841d255cbf87a4a759b36d223d3c7cb15220919a152155f303286f0",
}
...
GUIDE_HOW = [
    ("1", "Opři se o dlaně a kolena. Dlaně polož pod ramena, trup drž pevný a chodidla nech uvolněná."),
    ("2", "S nádechem pokrč lokty šikmo dozadu a kontrolovaně spouštěj hrudník k podložce."),
    ("3", "Pánev drž v ose s trupem. Neprohýbej bedra ani nezvedej boky."),
    ("4", "S výdechem zatlač do dlaní a vrať se do výchozí polohy."),
]
GUIDE_WATCH = (
    "Nepropadej se v bedrech a nezvedej pánev. Lokty nerozevírej do stran ani je netiskni těsně k tělu. "
    "Hlava zůstává v prodloužení páteře."
)
STEP_TEXTS = [
    ("KROK 1", "VÝCHOZÍ POLOHA", START,
     "Opři se o dlaně a kolena. Dlaně měj pod rameny, střed těla aktivní a hlavu v prodloužení páteře."),
    ("KROK 2", "POHYB DOLŮ", HERO,
     "S nádechem pokrč lokty šikmo dozadu a kontrolovaně spouštěj hrudník k podložce."),
    ("KROK 3", "PEVNÁ OSA TĚLA", HERO,
     "Pánev drž v jedné ose s trupem. Neprohýbej bedra, nezvedej boky a nepokládej hrudník na podložku."),
    ("KROK 4", "NÁVRAT VZHŮRU", START,
     "S výdechem zatlač do dlaní, propni paže bez zamykání loktů a vrať se do výchozí polohy."),
]
STEP_BREATH = "Nadechni se při pohybu dolů. Vydechni při vytlačení zpět vzhůru."
STEP_WATCH = (
    "Trup drž pevný od kolen až k hlavě. Lokty směřují šikmo dozadu, ramena zůstávají od uší "
    "a ruce jsou stabilně pod rameny."
)
...
def draw_wrapped(draw, xy, text, selected_font, fill, max_width, line_gap=7):
    ...
    for line in wrap_text(draw, text, selected_font, max_width):
        ...
        box = draw.textbbox((x, y), line, font=selected_font)
        last_bottom = box[3]
        y += draw.textbbox((0, 0), line, font=selected_font)[3] + line_gap
    return y, last_bottom


def fit_image(path, size, centering=(0.5, 0.62)):
...
def build_guide():
    ...
    draw.text((62, 47), "KNEE PUSH-UP", font=F["title"], fill=INK)
    draw.text((62, 91), "Klik na kolenou", font=F["small_b"], fill=TEAL_D)
    _, bottom = draw_wrapped(draw, (62, 114), "Posílení paží, hrudníku a středu těla.", F["tiny"], MUTED, 650, 2)
    margins["GUIDE DESCRIPTION"] = 140 - bottom
    x = pill(draw, (62, 148), "Horní část těla", F["small_b"])
    ...
    labels = [
        ...
        ("DOLŮ", "Kontrolovaně k podložce", HERO),
        ("ZPĚT", "Vytlač se vzhůru", START),
    ]
    ...
    info = [
        ("breath", "DECH", "Nádech při pohybu dolů, výdech při vytlačení vzhůru."),
        ("focus", "ZAMĚŘ SE", "Pevný střed, stabilní pánev a lokty šikmo dozadu."),
        ("repeat", "OPAKOVÁNÍ", "Prováděj pomalu podle předepsaného počtu opakování."),
    ]
...
def build_step():
    ...
    draw.text((62, 96), "KNEE PUSH-UP • KLIK NA KOLENOU", font=F["small_b"], fill=TEAL_D)
    ...
    for step_label, heading, source, body in STEP_TEXTS:
        ...
        image_bottom = y + 350
        paste_round(image, fit_image(source, (656, 240)), (62, y + 110, 718, image_bottom), 20)
