...
START = BASE / "hamstring_stretch_start_v01.png"
HERO = BASE / "hamstring_stretch_hero_v01.png"
GUIDE = BASE / "hamstring_stretch_guide_card_v01.png"
STEP = BASE / "hamstring_stretch_step_by_step_v01.png"
EXPECTED_HASHES = {
    START: "b2d29d30cef09c0304404227fec6025e0a95f214515b354951e2dfa48035b947",
    HERO: "b773e3f2f861aa4ef65d5266bb4f6e688e1d86c40371b5f24d065f18cfe1e094",
}
...
GUIDE_HOW = [
    ("1", "Lehni si na záda. Jedno koleno pokrč a chodidlo polož celé na podložku."),
    ("2", "Druhé stehno přitáhni k trupu a oběma rukama ho podepři zezadu nad kolenem."),
    ("3", "S výdechem pomalu propínej pracovní koleno. S nádechem ho kontrolovaně znovu pokrč."),
]

GUIDE_WATCH = (
    "Nezvedej pánev ani ramena, netlač rukama do kolena a nepropínej nohu přes bolest."
)
...
STEP_TEXTS = [
    (
        "KROK 1",
        "VÝCHOZÍ POZICE",
        "Lehni si na záda. Jedno koleno pokrč, chodidlo polož na podložku a druhé stehno podepři oběma rukama zezadu nad kolenem.",
        START,
    ),
    (
        "KROK 2",
        "PROPNUTÍ NOHY",
        "S výdechem pomalu propínej pracovní koleno a směřuj patu vzhůru. Propni pouze do příjemného tahu.",
        HERO,
    ),
    (
        "KROK 3",
        "KONTROLOVANÝ NÁVRAT",
        "S nádechem pracovní koleno kontrolovaně znovu pokrč. Stehno i pánev drž ve stejné poloze.",
        START,
    ),
]

STEP_BREATH = "Výdech při propnutí pracovní nohy. Nádech při kontrolovaném návratu."
STEP_WATCH = (
    "Nezvedej pánev ani ramena, netlač rukama do kolena a nepropínej nohu přes bolest."
)
...
def build_guide():
    ...
    draw.text((62, 49), "JEMNÉ PROTAŽENÍ ZADNÍ STRANY STEHNA", font=F["title"], fill=INK)
    draw.text((62, 84), "Hamstring Stretch", font=F["small_b"], fill=TEAL_D)
    _, desc_bottom = draw_wrapped(
        draw, (62, 104), "Jemně protahuje zadní stranu stehna a zlepšuje pohyblivost kolene.",
        F["tiny"], MUTED, 650, 2
    )
    margins["GUIDE DESCRIPTION"] = 140 - desc_bottom
    x = pill(draw, (62, 148), "Zadní strana stehna", F["small_b"])
    ...
    xs = [34, 274, 514]
    labels = [
        ("START", "Koleno pokrčené", START),
        ("PROPNUTÍ", "Pata vzhůru", HERO),
    ...
    info = [
        ("breath", "DECH", "Výdech při propnutí. Nádech při návratu."),
        ("focus", "ZAMĚŘ SE", "Pánev drž klidnou. Pohyb veď pouze v koleni."),
        ("repeat", "OPAKOVÁNÍ", "Drž podle dávky uvedené v tréninku."),
    ]
...
def build_step():
    ...
    draw.text((62, 98), "JEMNÉ PROTAŽENÍ ZADNÍ STRANY STEHNA / Hamstring Stretch", font=F["small_b"], fill=TEAL_D)
