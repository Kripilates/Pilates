...
START = BASE / "swimming_start_v01.png"
HERO = BASE / "swimming_hero_v01.png"
GUIDE = BASE / "swimming_guide_card_v01.png"
STEP = BASE / "swimming_step_by_step_v01.png"
...
def fit_image(path, size, crop=True):
    with Image.open(path) as source:
        ...
        if crop:
            return ImageOps.fit(source, size, method=Image.Resampling.LANCZOS, centering=(0.5, 0.55))
...
def build_guide():
    ...
    draw.text((62, 48), "SWIMMING", font=F["title"], fill=INK)
    draw.text((62, 98), "Střídavé posílení zad a středu těla", font=F["subtitle"], fill=TEAL_D)
    x = pill(draw, (62, 154), "Záda + střed těla", F["small_b"])
    pill(draw, (x, 154), "Bez pomůcky", F["small_b"])
    ...
    xs = [34, 274, 514]
    labels = [
        ("START", "Leh na břiše", START),
        ("HERO", "Zvedni protilehlé končetiny", HERO),
        ("START", "Plynule střídej", START),
    ]
    ...
    info = [
        ("Dech", ["Dýchej plynule.", "Nezadržuj dech."], "breath"),
        ("Zaměř se", ["Drž střed aktivní.", "Ramena dolů."], "focus"),
        ("Opakování", ["Lehká/Střední 20-30 s", "Náročná 30-35 s"], "repeat"),
    ]
    ...
    guide_lines = [
        "1. Lehni si na břicho a natáhni paže před sebe.",
        "2. Lehce zpevni břicho a prodluž celé tělo.",
        "3. Zvedni jednu paži a protilehlou nohu.",
        "4. Plynule střídej strany bez houpání trupu.",
    ]
    y = 1230
    ...
    draw_wrapped(
        draw,
        (62, 1544),
        "Nezakláněj hlavu a nezvedej končetiny příliš vysoko. Pohyb veď kontrolovaně a drž pánev stabilní.",
        F["body"],
        MUTED,
        650,
        7,
...
def build_step():
    ...
    draw.text((62, 94), "Swimming", font=F["small_b"], fill=TEAL_D)

    steps = [
        ("START", START, "Lehni si na břicho. Paže natáhni před sebe, nohy dozadu a drž krk v prodloužení páteře."),
        ("ZVEDNUTÍ", HERO, "Zvedni jednu paži a protilehlou nohu jen několik centimetrů nad podložku."),
        ("STŘÍDÁNÍ", HERO, "Plynule vystřídej opačnou paži a nohu. Trup a pánev drž co nejstabilnější."),
        ("KONTROLA", START, "Pokračuj v klidném střídání a po dokončení kontrolovaně polož ruce i nohy zpět."),
    ]
    ...
    draw_wrapped(
        draw,
        (62, 2186),
        "Dýchej plynule po celou dobu pohybu a nezadržuj dech.",
        F["body"],
        MUTED,
        650,
        7,
