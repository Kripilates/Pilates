...
START = BASE / "side_plank_reach_start_v01.png"
HERO = BASE / "side_plank_reach_hero_v01.png"
END = BASE / "side_plank_reach_end_v01.png"
GUIDE = BASE / "side_plank_reach_guide_card_v01.png"
STEP = BASE / "side_plank_reach_step_by_step_v01.png"
EXPECTED_HASHES = {
...
GUIDE_HOW = [
    ("1", "Nastav boční prkno na předloktí. Loket dej pod rameno, nohy natáhni a pánev drž vysoko."),
    ("2", "S výdechem veď horní paži pod trup a současně kontrolovaně rotuj hrudník k podložce."),
    ("3", "S nádechem otevři hrudník a vrať horní paži vzhůru. Pánev zůstává stabilní."),
]

GUIDE_WATCH = (
    "Nepropadej se v opěrném rameni a nenechávej pánev klesat. Rotuj kontrolovaně "
    "hrudníkem, ne švihem paže, a neprohýbej bedra."
)
...
STEP_TEXTS = [
    (
        "KROK 1",
        "VÝCHOZÍ POLOHA",
        "Nastav boční prkno na předloktí. Loket dej přímo pod rameno, nohy natáhni a pánev drž vysoko. Horní paže je podél těla.",
        START,
    ),
    (
        "KROK 2",
        "REACH POD TRUP",
        "S výdechem veď horní paži pod trup a současně kontrolovaně rotuj hrudník k podložce. Opěrné rameno zůstává aktivní.",
        HERO,
    ),
    (
        "KROK 3",
        "NÁVRAT",
        "S nádechem otevři hrudník a vrať horní paži vzhůru podle koncové polohy. Pánev drž stabilní a tělo v dlouhé linii.",
        END,
    ),
]

STEP_BREATH = "Vydechni při reach pod trup a rotaci. Nadechni se při kontrolovaném otevření a návratu."
STEP_WATCH = (
    "Loket drž pod ramenem. Nepropadej se v opěrném rameni, nenechávej klesat pánev "
    "a nerotuj švihem."
)
...
def build_guide():
    ...
    draw.text((62, 49), "BOČNÍ PRKNO S ROTACÍ", font=F["title"], fill=INK)
    draw.text((62, 94), "Side Plank Reach • střed těla • pas", font=F["small_b"], fill=TEAL_D)
    _, desc_bottom = draw_wrapped(
        draw, (62, 115), "Posiluje šikmé břišní svaly a stabilitu ramene i pánve.",
        F["tiny"], MUTED, 650, 2
    )
    ...
    xs = [34, 274, 514]
    labels = [
        ("START", "Stabilní boční prkno", START),
        ("REACH", "Paže pod trup", HERO),
        ("NÁVRAT", "Otevři hrudník", END),
    ]
    ...
    info = [
        ("breath", "DECH", "Výdech při rotaci, nádech při návratu."),
        ("focus", "ZAMĚŘ SE", "Pevný střed, pánev a aktivní rameno."),
        ("repeat", "OPAKOVÁNÍ", "6–12 na každou stranu podle tréninku."),
    ]
...
def build_step():
    ...
    draw.text((62, 98), "BOČNÍ PRKNO S ROTACÍ", font=F["small_b"], fill=TEAL_D)
    ...
    _, watch_bottom = draw_wrapped(draw, (62, next_y + 54), STEP_WATCH, F["step_body"], INK, 650, 5)
