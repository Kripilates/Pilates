...
START = BASE / "side_plank_start_v01.png"
HERO = BASE / "side_plank_hero_v01.png"
END = START
GUIDE = BASE / "side_plank_guide_card_v01.png"
STEP = BASE / "side_plank_step_by_step_v01.png"
EXPECTED_HASHES = {
...
GUIDE_HOW = [
    ("1", "Lehni si na bok, spodní loket dej přímo pod rameno a nohy natáhni. Pánev nech na podložce."),
    ("2", "S výdechem zpevni střed těla a zvedni pánev. Tělo vytáhni do dlouhé linie."),
    ("3", "S nádechem spusť pánev pomalu zpět na podložku. Potom vystřídej stranu."),
]

GUIDE_WATCH = (
    "Nepropadej se v rameni, netahej rameno k uchu a nenechávej pánev klesat. "
    "Bedra neprohýbej a pohyb veď kontrolovaně."
)
...
STEP_TEXTS = [
    (
        "KROK 1",
        "VÝCHOZÍ POLOHA",
        "Lehni si na bok. Spodní loket dej přímo pod rameno, předloktí opři o podložku a nohy natáhni. Pánev zůstává dole.",
        START,
    ),
    (
        "KROK 2",
        "SIDE PLANK",
        "S výdechem zpevni břicho a zvedni pánev z podložky. Tělo drž v dlouhé linii a spodní rameno aktivní.",
        HERO,
    ),
    (
        "KROK 3",
        "KONTROLOVANÝ NÁVRAT",
        "S nádechem spusť pánev pomalu zpět na podložku. Zachovej oporu předloktí a pohyb dokonči bez propadnutí v rameni.",
        END,
    ),
]

STEP_BREATH = "Dýchej plynule. Vydechni při zvednutí pánve, nadechni se při kontrolovaném návratu."
STEP_WATCH = (
    "Loket drž pod ramenem. Nepropadej se v rameni, nezvedej rameno k uchu, "
    "nenechávej pánev klesat a neprohýbej bedra."
)
...
def build_guide():
    ...
    draw.text((62, 49), "SIDE PLANK", font=F["title"], fill=INK)
    draw.text((62, 94), "Střed těla • pas • ramena", font=F["small_b"], fill=TEAL_D)
    _, desc_bottom = draw_wrapped(
        draw, (62, 115), "Posiluje střed těla a učí stabilní oporu ramene i pánve.",
        F["tiny"], MUTED, 650, 2
    )
    ...
    xs = [34, 274, 514]
    labels = [
        ("START", "Pánev na podložce", START),
        ("SIDE PLANK", "Zvedni pánev", HERO),
        ("NÁVRAT", "Kontrolovaně zpět", END),
    ]
    ...
    info = [
        ...
        ("focus", "ZAMĚŘ SE", "Pevný střed a aktivní rameno."),
        ("repeat", "VÝDRŽ", "Drž podle času v tréninku."),
    ]
...
def build_step():
    ...
    draw.text((62, 98), "SIDE PLANK", font=F["small_b"], fill=TEAL_D)
