...
GUIDE_HOW = [
    ("1", "Lehni si na břicho, nohy natáhni dozadu a předloktí polož na podložku."),
    ("2", "Lokty nastav přibližně pod ramena a jemně zvedni hrudník. Pánev a stehna nech na podložce."),
    ("3", "Ramena stáhni od uší, prodluž páteř a v pozici klidně dýchej."),
]

GUIDE_WATCH = (
    "Nevytahuj ramena k uším a nezakláněj hlavu. Nezvedej pánev ani stehna "
    "a neprohýbej bedra násilím."
)
...
STEP_TEXTS = [
    (
        "KROK 1",
        "NASTAVENÍ",
        "Lehni si na břicho a natáhni nohy dozadu. Předloktí polož na podložku a lokty nastav přibližně pod ramena.",
        START,
    ),
    (
        "KROK 2",
        "SPHINX",
        "Opři se do předloktí a jemně zvedni hrudník. Pánev a stehna nech na podložce a ramena drž daleko od uší.",
        HERO,
    ),
    (
        "KROK 3",
        "VÝDRŽ",
        "Prodluž páteř, krk nech přirozeně a v pozici klidně dýchej. Záklon zvětšuj jen do příjemného rozsahu.",
        END,
    ),
]

STEP_BREATH = "Dýchej pomalu a plynule. S nádechem prodlužuj páteř a otevírej hrudník."
STEP_WATCH = (
    "Ramena nevytahuj k uším, nezakláněj hlavu a netlač bedra do nepříjemného záklonu. "
    "Pánev a stehna zůstávají na podložce."
)
...
def build_guide():
    ...
    draw.text((62, 49), "SPHINX STRETCH", font=F["title"], fill=INK)
    draw.text((62, 94), "Záda • hrudník • mobilita páteře", font=F["small_b"], fill=TEAL_D)
    _, desc_bottom = draw_wrapped(
        draw, (62, 115), "Jemně otevři hrudník a protáhni přední stranu těla.",
        F["tiny"], MUTED, 650, 2
    )
    ...
    xs = [34, 274, 514]
    labels = [
        ("NASTAVENÍ", "Leh na břiše", START),
        ("SPHINX", "Opři se o předloktí", HERO),
        ("VÝDRŽ", "Uvolni ramena", END),
    ]
    ...
    info = [
        ("breath", "DECH", "Dýchej pomalu. S nádechem prodlužuj páteř."),
        ("focus", "ZAMĚŘ SE", "Ramena od uší a dlouhá páteř."),
        ("repeat", "VÝDRŽ", "Drž podle dávky v aktuálním tréninku."),
    ]
...
def build_step():
    ...
    draw.text((62, 98), "SPHINX STRETCH", font=F["small_b"], fill=TEAL_D)
