...
START = BASE / "glute_bridge_start_v02_approved.png"
HERO = BASE / "glute_bridge_hero_v02_approved.png"
...
EXPECTED_HASHES = {
    ...
    HERO: "961e86918b529a1eb94dab1fb551391970c88d8b7f076fae5030b5ae369d6ebd",
}
...
GUIDE_HOW = [
    ...
    ("2", "S výdechem aktivuj hýždě a zvedni pánev do rovné linie ramen, boků a kolen."),
    ("3", "S nádechem polož pánev pomalu zpět na podložku a udrž pohyb kontrolovaný."),
]

GUIDE_WATCH = (
    "Aktivuj hýždě a střed těla. Neprohýbej bedra, nezvedej ramena a netlač pohyb přes spodní záda."
)
...
STEP_TEXTS = [
    ...
    (
        "KROK 2",
        "HLAVNÍ POHYB",
        "S výdechem zvedni pánev nahoru. Hýždě aktivuj a udrž tělo v dlouhé stabilní linii.",
        HERO,
    ),
    (
        "KROK 3",
        "NÁVRAT",
        "Pomalu a kontrolovaně polož pánev zpět do výchozí polohy.",
        START,
    ),
]

STEP_BREATH = "Výdech při zvedání pánve. Nádech při návratu zpět na podložku."
STEP_WATCH = (
    "Aktivuj hýždě a střed těla. Neprohýbej bedra, netlač žebra nahoru a pohyb veď pomalu."
)
...
def build_guide():
    ...
    draw.text((62, 52), "GLUTE BRIDGE", font=F["title"], fill=INK)
    draw.text((62, 98), "Most pro aktivaci hýždí", font=F["small_b"], fill=TEAL_D)
    _, desc_bottom = draw_wrapped(
        draw,
        (62, 118),
        "Kontrolované zvednutí pánve pro hýždě a střed těla.",
        F["tiny"],
        MUTED,
        650,
        2,
    ...
    labels = [
        ...
        ("HERO", "Zvedni pánev", HERO),
        ("NÁVRAT", "Polož pánev", START),
    ]
    ...
    info = [
        ("breath", "DECH", "Výdech při zvedání. Nádech při návratu."),
        ("focus", "ZAMĚŘ SE", "Aktivuj hýždě a střed těla. Neprohýbej bedra."),
        ("repeat", "OPAKOVÁNÍ", "12 opakování 2-3 série"),
    ]
...
def build_step():
    ...
    draw.text((62, 98), "GLUTE BRIDGE", font=F["small_b"], fill=TEAL_D)
