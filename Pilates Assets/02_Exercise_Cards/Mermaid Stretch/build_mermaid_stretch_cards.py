...
START = BASE / "mermaid_stretch_start_v01.png"
HERO = BASE / "mermaid_stretch_hero_v01.png"
GUIDE = BASE / "mermaid_stretch_guide_card_v01.png"
STEP = BASE / "mermaid_stretch_step_by_step_v01.png"
EXPECTED_HASHES = {
    START: "dd9ebd6a75d5f397842ffd75b3196bbeebd0e6797fb9ba4ba96aa9308c66592f",
    HERO: "6105f3dd084aac7680d4437fb7334977c6f8522ccac2223457526c6b23f9c759",
}
...
GUIDE_HOW = [
    ("1", "Sedni si do Mermaid sedu. Obě kolena pokrč na jednu stranu a páteř vytáhni vzhůru."),
    ("2", "Jednu dlaň lehce opři o podložku. S výdechem se ukloň a druhou paži veď nad hlavou."),
    ("3", "S nádechem se plynule vrať do vzpřímeného sedu. Pánev zůstává stabilní."),
]

GUIDE_WATCH = (
    "Sedací kosti drž co nejvíce na podložce, ramena daleko od uší a hrudník dopředu. "
    "Neotáčej trup, nepředkláněj se a protahuj se jen do příjemného rozsahu."
)
...
STEP_TEXTS = [
    (
        "KROK 1",
        "VÝCHOZÍ POZICE",
        "Sedni si vzpřímeně, obě kolena pokrč na jednu stranu. Jednu ruku lehce opři o podložku.",
        START,
    ),
    (
        "KROK 2",
        "BOČNÍ ÚKLON",
        "S výdechem se ukláněj k opěrné ruce. Opačnou paži protáhni obloukem nad hlavou.",
        HERO,
    ),
    (
        "KROK 3",
        "NÁVRAT",
        "S nádechem se plynule vrať do vzpřímeného sedu. Pánev zůstává stabilní.",
        START,
    ),
]

STEP_BREATH = "Nádech v sedu. Výdech do úklonu. Nádech při návratu."
STEP_WATCH = (
    "Sedací kosti drž na podložce a ramena daleko od uší. "
    "Neotáčej trup ani se nepředkláněj."
)
...
def build_guide():
    ...
    draw.text((62, 49), "BOČNÍ PROTAŽENÍ V SEDU", font=F["title"], fill=INK)
    draw.text((62, 94), "Bok trupu a mobilita páteře", font=F["small_b"], fill=TEAL_D)
    _, desc_bottom = draw_wrapped(
        draw, (62, 115), "Protahuje bok trupu a podporuje vzpřímené držení těla.",
        F["tiny"], MUTED, 650, 2
    )
    margins["GUIDE DESCRIPTION"] = 140 - desc_bottom
    x = pill(draw, (62, 148), "Boční protažení", F["small_b"])
    ...
    labels = [
        ...
        ("BOČNÍ ÚKLON", "Paže nad hlavou", HERO),
        ("NÁVRAT", "Plynule zpět", START),
    ]
    ...
    info = [
        ("breath", "DECH", "Výdech do úklonu. Nádech při návratu."),
        ("focus", "ZAMĚŘ SE", "Pánev stabilní, ramena daleko od uší."),
        ("repeat", "VÝDRŽ", "Drž podle času uvedeného v tréninku."),
    ]
...
def build_step():
    ...
    draw.text((62, 98), "BOČNÍ PROTAŽENÍ V SEDU", font=F["small_b"], fill=TEAL_D)
