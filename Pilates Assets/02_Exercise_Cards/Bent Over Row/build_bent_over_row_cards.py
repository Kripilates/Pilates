...
def paste_photo(canvas, img, xy, size, radius=20, y_bias=0.2):
...
def draw_guide():
    ...
    d.text((63, 63), "BENT OVER ROW", font=f(41, True), fill=INK)
    d.text(
        (64, 111),
        "Posiluje horní část zad, lopatky a zadní ramena.",
        font=f(15),
        fill=MUTED,
    )
    pill(d, 63, 145, "Lehké činky")
    pill(d, 216, 145, "Záda a lopatky")

    card(d, (34, 198, 746, 653), radius=28)
    paste_photo(im, hero, (50, 218), (680, 393), radius=22, y_bias=0.22)

    mini = [
        ("1", "START", "Činky pod rameny", start),
        ("2", "PŘÍTAH", "Lokty dozadu", hero),
        ("3", "NÁVRAT", "Kontrolovaně dolů", start),
    ]
    mini_y, mini_w, mini_h, gap = 684, 220, 194, 20
    for idx, (num, title, body, pic) in enumerate(mini):
        x = 34 + idx * (mini_w + gap + 20)
        if idx == 1:
            x = 280
        elif idx == 2:
            x = 526
        card(d, (x, mini_y, x + mini_w, mini_y + mini_h), radius=20)
        paste_photo(im, pic, (x + 12, mini_y + 12), (mini_w - 24, 104), radius=14, y_bias=0.15)
    ...
    infos = [
        ("breath", "DECH", "Výdech při přítahu.\nNádech při návratu."),
        ("focus", "ZAMĚŘ SE", "Záda dlouhá, ramena\ndaleko od uší."),
        ("repeat", "OPAKOVÁNÍ", "Plynule podle dávky\nv tréninku."),
    ]
    ...
    steps = [
        "Postav se na šířku kyčlí, lehce pokrč kolena a posuň boky dozadu.",
        "S výdechem veď lokty dozadu těsně kolem trupu a přitáhni činky k dolním žebrům.",
        "S nádechem spouštěj činky kontrolovaně zpět pod ramena bez změny úhlu trupu.",
    ]
    y = 1170
    ...
    draw_wrap(
        d,
        (63, 1510),
        "Nekulať ani nadměrně neprohýbej záda. Nezvedej ramena k uším, neroztahuj lokty do stran a nezalamuj zápěstí.",
        f(21),
        INK,
        635,
        gap=8,
...
def draw_step():
    ...
    d.text((56, 93), "BENT OVER ROW", font=f(14, True), fill=TURQ_DARK)

    rows = [
        ("KROK 1", "START", "Postav se na šířku kyčlí. Kolena lehce pokrč, boky posuň dozadu a předkloň se z kyčlí. Paže nech natažené, činky pod rameny.", start),
        ("KROK 2", "PŘÍTAH", "S výdechem veď lokty dozadu těsně kolem trupu. Přitáhni činky k dolním žebrům a lopatky jemně stáhni k sobě a dolů.", hero),
        ("KROK 3", "NÁVRAT", "S nádechem spouštěj činky kontrolovaně dolů. Natáhni paže zpět pod ramena a udrž stabilní pánev i neutrální záda.", start),
    ]
    y = 145
    for label, title, body, pic in rows:
        ...
        paste_photo(im, pic, (56, y + 132), (648, 300), radius=18, y_bias=0.28)
    ...
    draw_wrap(d, (56, 1963), "Výdech při přítahu. Nádech při návratu.", f(21), INK, 645, gap=8)
    draw_warning_icon(d, 56, 2035, 22)
    d.text((92, 2032), "HLÍDEJ SI", font=f(22, True), fill=INK)
    draw_wrap(
        d,
        (56, 2084),
        "Nešvihej trupem, neprováděj bicepsový zdvih a nezalamuj zápěstí. Ramena drž daleko od uší a lokty netahej široce do stran.",
        f(21),
        INK,
        645,
        gap=8,
...
if __name__ == "__main__":
    ...
    if target in ("all", "guide"):
        draw_guide()
    if target in ("all", "step"):
        draw_step()
