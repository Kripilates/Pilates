...
GUIDE_HOW = [
    ("1", "Postav se vzpřímeně, chodidla dej přibližně na šířku kyčlí a paže nech volně podél těla."),
    ("2", "S nádechem spoj ruce za tělem, ramena stáhni dolů a dozadu a jemně otevři hrudník."),
    ("3", "S výdechem pomalu uvolni ruce a vrať paže kontrolovaně do výchozí polohy."),
]

GUIDE_WATCH = (
    "Nezvedej ramena k uším, nezakláněj hlavu a neotevírej hrudník "
    "prohnutím v bedrech."
)
...
STEP_TEXTS = [
    (
        ...
        "START",
        START,
        "Postav se vzpřímeně, chodidla dej přibližně na šířku kyčlí a paže nech volně podél těla.",
    ),
    (
        "KROK 2",
        "OTEVŘENÍ HRUDNÍKU",
        HERO,
        "S nádechem spoj ruce za tělem. Ramena stáhni dolů a dozadu a jemně otevři hrudník.",
    ),
    (
        "KROK 3",
        "NÁVRAT",
        START,
        "S výdechem pomalu uvolni ruce a vrať paže kontrolovaně do výchozí polohy.",
    ),
]

STEP_BREATH = "Nádech při otevření hrudníku. Výdech při návratu."
STEP_WATCH = (
    "Žebra nevytlačuj dopředu. Neprohýbej bedra, nezvedej ramena k uším "
    "a hlavu drž v neutrální poloze."
)
...
def build_guide():
    ...
    draw.text((62, 52), "OTEVŘENÍ HRUDNÍKU", font=F["title"], fill=INK)
    draw.text((62, 98), "Chest Opener", font=F["small_b"], fill=TEAL_D)
    draw_wrapped(
        draw,
        (62, 118),
        "Uvolňuje hrudník a ramena. Podporuje vzpřímené držení těla.",
        F["tiny"],
        MUTED,
        650,
        2,
    )
    x = pill(draw, (62, 142), "Mobilita hrudníku", F["small_b"])
    pill(draw, (x, 142), "Uvolnění ramen", F["small_b"])
    ...
    labels = [
        ("START", "Paže podél těla"),
        ("OTEVŘENÍ", "Ruce spoj za tělem"),
        ("NÁVRAT", "Pomalu uvolni"),
    ]
    ...
    info = [
        ...
        ("focus", "ZAMĚŘ SE", "Ramena dolů a dozadu. Žebra drž klidná."),
        ("repeat", "OPAKOVÁNÍ", "Drž podle dávky v tréninku."),
    ]
...
def build_step():
    ...
    draw.text((62, 98), "OTEVŘENÍ HRUDNÍKU / Chest Opener", font=F["small_b"], fill=TEAL_D)
