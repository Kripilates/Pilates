...
START = BASE / "rainbow_leg_raise_start_v02.png"
HERO = BASE / "rainbow_leg_raise_hero_v02.png"
END = BASE / "rainbow_leg_raise_end_v02.png"
GUIDE = BASE / "rainbow_leg_raise_guide_card_v01.png"
STEP = BASE / "rainbow_leg_raise_step_by_step_v01.png"

EXPECTED_HASHES = {
    START: "0b071c6d1f5b7b636a9e5ca179184774bce99ed425bcc35fe0234e411220b302",
    HERO: "de91293997a86bc546d04bb5342cb94112b0db2e66df98d62014e42a3047e760",
    END: "accb55e750e2293cd8205d11feb46b27e57df75783b57acacb2568073a3d8bf5",
}
...
GUIDE_HOW = [
    ("1", "Začni na všech čtyřech. Jednu pracovní nohu natáhni nízko do strany a zpevni střed těla."),
    ("2", "Stejnou pracovní nohu veď plynulým obloukem nahoru přes střed. Pánev drž stabilní."),
    ("3", "Dokonči oblouk stejnou nohou za opěrnou nohou a vrať se kontrolovaně bez švihu."),
]

GUIDE_WATCH = (
    "Pohyb po celou dobu vede stejná pracovní noha. Nepřetáčej pánev, neprohýbej bedra "
    "a nezvedej nohu výš za cenu ztráty kontroly."
)
...
STEP_TEXTS = [
    (
        "KROK 1",
        "VÝCHOZÍ POLOHA",
        "Začni na všech čtyřech. Jednu pracovní nohu natáhni nízko do výchozí strany a drž pánev stabilní.",
        START,
    ),
    (
        "KROK 2",
        "OBLOUK NAHORU",
        "Stejnou pracovní nohu zvedej plynulým obloukem nahoru přes střed. Pohyb veď z kyčle, bez švihu.",
        HERO,
    ),
    (
        "KROK 3",
        "DOKONČENÍ OBLOUKU",
        "Dokonči oblouk stejnou pracovní nohou za opěrnou nohou nízko na opačné straně. Pánev zůstává co nejklidnější.",
        END,
    ),
]

STEP_BREATH = "Dýchej plynule. Vydechuj při vedení nohy obloukem a nadechni se při kontrolovaném návratu."
STEP_WATCH = (
    "Pracuje stále stejná noha. Nepřetáčej pánev, neklesej do ramen a neprohýbej bedra."
)
...
def build_guide():
    ...
    draw.text((62, 49), "RAINBOW LEG RAISE", font=F["title"], fill=INK)
    draw.text((62, 94), "Hýždě • boky • stabilita pánve", font=F["small_b"], fill=TEAL_D)
    _, desc_bottom = draw_wrapped(
        draw,
        (62, 115),
        "Kontrolovaný oblouk jedné natažené nohy ze strany přes střed za opěrnou nohu.",
        F["tiny"],
        MUTED,
        650,
        2,
    ...
    x = pill(draw, (62, 148), "Hýždě + boky", F["small_b"])
    ...
    xs = [34, 274, 514]
    labels = [
        ("START", "Noha nízko do strany", START),
        ("HERO", "Oblouk nahoru", HERO),
        ("END", "Za opěrnou nohu", END),
    ]
    for index, (x0, (label, caption, source)) in enumerate(zip(xs, labels), 1):
        ...
        paste_round(
            image,
            contain_image(source, (mini_w - 22, mini_h), 4),
    ...
    info = [
        ("breath", "DECH", "Výdech při oblouku. Nádech při návratu."),
        ("focus", "ZAMĚŘ SE", "Pracuje stejná noha, pánev zůstává klidná."),
        ("repeat", "OPAKOVÁNÍ", "8–14 na každou stranu podle tréninku."),
    ]
...
def build_step():
    ...
    draw.text((62, 98), "RAINBOW LEG RAISE", font=F["small_b"], fill=TEAL_D)
    ...
    for step_label, heading, body, source in STEP_TEXTS:
        ...
        image_bottom = y + 466
        paste_round(image, contain_image(source, (656, 352), 6), (62, y + 114, 718, image_bottom))
