...
START = BASE / "thread_the_needle_start.png"
HERO = BASE / "thread_the_needle_hero.png"
...
def build_guide():
    ...
    draw.text((62, 48), "THREAD THE NEEDLE", font=F["title"], fill=INK)
    draw.text((62, 98), "Protažení s rotací v kleku", font=F["subtitle"], fill=TEAL_D)
    x = pill(draw, (62, 154), "Mobilita zad", F["small_b"])
    ...
    xs = [34, 274, 514]
    labels = [
        ("START", "Na všech čtyřech", START),
        ("HERO", "Provleč paži", HERO),
        ("START", "Vrať se zpět", START),
    ]
    ...
    info = [
        ("Dech", ["Výdech při rotaci.", "Nádech při návratu."], "breath"),
        ("Zaměř se", ["Pánev drž nad koleny.", "Ramena nech uvolněná."], "focus"),
        ("Opakování", ["20 s / strana", "2-3 série"], "repeat"),
    ]
    for x0, (title, lines, icon) in zip(xs, info):
        ...
        y = info_y + 58
        for line in lines:
            draw.text((x0 + 18, y), line, font=F["tiny"], fill=MUTED)
            y += 27

    rounded(draw, (34, 1152, 746, 1436))
    draw.text((62, 1184), "Krátký průběh", font=F["h2"], fill=INK)
    guide_lines = [
        "1. Začni na všech čtyřech s dlaněmi pod rameny.",
        "2. S výdechem provleč jednu paži pod tělem.",
        "3. Nech rameno a hlavu jemně klesnout k podložce.",
        "4. S nádechem se vrať a vystřídej stranu.",
    ]
    y = 1230
    for line in guide_lines:
        y = draw_wrapped(draw, (62, y), line, F["body"], MUTED, 650, 5) + 5

    rounded(draw, (34, 1466, 746, 1648), 28, fill=(255, 247, 244), outline=(242, 220, 216), width=1)
    draw.text((62, 1498), "Na co si dát pozor", font=F["h2"], fill=INK)
    draw_wrapped(
        draw,
        (62, 1544),
        "Nevytáčej pánev a netlač rameno ani hlavu silou do podložky. Pohyb má zůstat pomalý a příjemný.",
        F["body"],
        MUTED,
        650,
        7,
...
def build_step():
    ...
    draw.text((62, 94), "Thread the Needle", font=F["small_b"], fill=TEAL_D)
...
if __name__ == "__main__":
    ...
    for path in (START, HERO, GUIDE, STEP):
        with Image.open(path) as im:
            print(f"{path.name}: {im.size} {im.mode} {sha256(path)}")
