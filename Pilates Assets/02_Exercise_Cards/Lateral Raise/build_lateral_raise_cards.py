...
START = BASE / "lateral_raise_start_v01.png"
HERO = BASE / "lateral_raise_hero_v01.png"
GUIDE = BASE / "lateral_raise_guide_card_v01.png"
STEP = BASE / "lateral_raise_step_by_step_v01.png"
EXPECTED_HASHES = {
    START: "2c4cbc24c0576825da202e25210b4464302c4d771ab0da85c1e65bd964569075",
    HERO: "6acbfab857b3a59e4fb13136aef75f07666c96c02d9dbf885a77d151faced9e9",
}
...
GUIDE_HOW = [
    ("1", "Postav se vzpřímeně. Lehké činky drž podél těla a lokty nech mírně pokrčené."),
    ("2", "Se zpevněným středem zvedni paže do stran přibližně do výšky ramen."),
    ("3", "Pomalu spusť paže zpět. Pohyb kontroluj po celou dobu."),
]

GUIDE_WATCH = (
    "Nezvedej ramena k uším, neprohýbej bedra a nepomáhej si švihem. "
    "Paže nezvedej výš než do úrovně ramen."
)
...
STEP_TEXTS = [
    (
        "KROK 1",
        "VÝCHOZÍ POZICE",
        "Postav se vzpřímeně. Do každé ruky vezmi lehkou činku. Paže jsou podél těla, lokty mírně pokrčené.",
        START,
    ),
    (
        "KROK 2",
        "UPAŽENÍ",
        "Se zpevněným středem těla zvedni paže do stran přibližně do výšky ramen. Ramena drž dole a pohyb veď plynule.",
        HERO,
    ),
    (
        "KROK 3",
        "KONTROLOVANÝ NÁVRAT",
        "Pomalu spusť paže zpět do výchozí polohy. Pohyb kontroluj po celou dobu.",
        START,
    ),
]

STEP_BREATH = "Výdech při zvednutí paží. Nádech při kontrolovaném návratu."
STEP_WATCH = (
    "Ramena drž daleko od uší, trup vzpřímený a bedra neutrální. "
    "Nešvihej činkami a nezvedej paže nad úroveň ramen."
)
...
def build_guide():
    ...
    draw.text((62, 49), "UPAŽOVÁNÍ S ČINKAMI", font=F["title"], fill=INK)
    draw.text((62, 94), "Ramena a stabilita trupu", font=F["small_b"], fill=TEAL_D)
    _, desc_bottom = draw_wrapped(
        draw, (62, 115), "Posiluje ramena a učí kontrolovaný pohyb bez švihu.",
        F["tiny"], MUTED, 650, 2
    )
    margins["GUIDE DESCRIPTION"] = 140 - desc_bottom
    x = pill(draw, (62, 148), "Ramena", F["small_b"])
    pill(draw, (x, 148), "Lehké činky", F["small_b"])
    ...
    xs = [34, 274, 514]
    labels = [
        ("START", "Činky podél těla", START),
        ("UPAŽENÍ", "Paže do výšky ramen", HERO),
    ...
    info = [
        ...
        ("focus", "ZAMĚŘ SE", "Ramena drž dole a střed těla zpevněný."),
...
def build_step():
    ...
    draw.text((62, 98), "UPAŽOVÁNÍ S ČINKAMI", font=F["small_b"], fill=TEAL_D)
