...
GUIDE_HOW = [
    ("1", "Sedni si vzpřímeně, nohy natáhni před sebe a chodidla dej do flexe. Paže natáhni vpřed ve výšce ramen."),
    ("2", "S výdechem skloň hlavu a postupně zakulacuj páteř od horní části zad. Paže veď vpřed."),
    ("3", "S nádechem se kontrolovaně vrať obratel po obratli do vzpřímeného sedu."),
]

GUIDE_WATCH = (
    "Nevytahuj ramena k uším a nepřetahuj pohyb silou. Pánev drž stabilní a pohyb veď "
    "zakulacením páteře, ne pouhým předklonem z kyčlí."
)
...
STEP_TEXTS = [
    (
        "KROK 1",
        "VÝCHOZÍ POLOHA",
        "Sedni si vzpřímeně, nohy natáhni před sebe a chodidla dej do flexe. Paže natáhni vpřed ve výšce ramen.",
        START,
    ),
    (
        "KROK 2",
        "ZAKULACENÍ PÁTEŘE",
        "S výdechem skloň hlavu a postupně zakulacuj páteř. Paže veď vpřed a ramena drž daleko od uší.",
        HERO,
    ),
    (
        "KROK 3",
        "KONTROLOVANÝ NÁVRAT",
        "S nádechem se postupně vrať do vzpřímeného sedu. Páteř znovu vytáhni vzhůru a zachovej stabilní pánev.",
        END,
    ),
]
STEP_BREATH = "Vydechni při zakulacení a pohybu vpřed. Nadechni se při kontrolovaném návratu."
STEP_WATCH = (
    "Pohyb veď páteří, ne pouze předklonem z kyčlí. Ramena drž uvolněná, pánev stabilní "
    "a nechoď do rozsahu silou."
)
...
def build_guide():
    ...
    draw.text((62, 49), "SPINE STRETCH", font=F["title"], fill=INK)
    draw.text((62, 94), "Záda • střed těla • mobilita páteře", font=F["small_b"], fill=TEAL_D)
    _, desc_bottom = draw_wrapped(
        draw, (62, 115), "Kontrolovaně zakulať páteř a protáhni ji směrem vpřed.",
        F["tiny"], MUTED, 650, 2
    )
    margins["GUIDE DESCRIPTION"] = 140 - desc_bottom
    x = pill(draw, (62, 148), "Mobilita / protažení", F["small_b"])
    ...
    xs = [34, 274, 514]
    labels = [
        ("START", "Vzpřímený sed", START),
        ("PROTAŽENÍ", "Zakulať páteř", HERO),
        ("NÁVRAT", "Zpět do vzpřímení", END),
    ]
    ...
    info = [
        ("breath", "DECH", "Vydechni při zakulacení a pohybu vpřed. Nadechni se při návratu."),
        ("focus", "ZAMĚŘ SE", "Plynulé zakulacení páteře a uvolněná ramena."),
        ("repeat", "OPAKOVÁNÍ", "Podle dávky v aktuálním tréninku."),
    ]
...
def build_step():
    ...
    draw.text((62, 98), "SPINE STRETCH", font=F["small_b"], fill=TEAL_D)
    ...
    next_y, breath_bottom = draw_wrapped(draw, (62, y + 74), STEP_BREATH, F["step_body"], INK, 650, 8)
