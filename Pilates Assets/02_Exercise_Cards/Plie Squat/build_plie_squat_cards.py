...
START = BASE / "plie_squat_start_v01.png"
HERO = BASE / "plie_squat_hero_v01.png"
GUIDE = BASE / "plie_squat_guide_card_v01.png"
STEP = BASE / "plie_squat_step_by_step_v01.png"
EXPECTED_HASHES = {
    START: "777f6ead33d005eae8c908547b6940fd095c0e6c5fea6b4be4bfba1ec3462cd2",
    HERO: "68a28653cc4b35bbb6381c4f0788dddfbd90e8f4fdcce735511beb0fa8220d82",
}
...
GUIDE_HOW = [
    ("1", "Postav se do širokého postoje a špičky vytoč mírně ven. Pánev drž neutrální."),
    ("2", "S nádechem pokrč kolena ve směru špiček a klesni do bezpečného rozsahu."),
    ("3", "S výdechem zatlač chodidla do podložky a vrať se plynule do stoje."),
]

GUIDE_WATCH = (
    "Nevtáčej kolena dovnitř, nezvedej paty a neklesej níž, než dovolí bezpečný "
    "rozsah. Trup drž vzpřímený."
)
...
STEP_TEXTS = [
    (
        "KROK 1",
        "VÝCHOZÍ POSTOJ",
        "Postav se do širokého postoje. Špičky vytoč mírně ven, paty nech na podložce, pánev drž neutrální a trup vzpřímený.",
        START,
    ),
    (
        "KROK 2",
        "PLIÉ DŘEP",
        "S nádechem pokrč kolena ve směru špiček. Klesej plynule jen do bezpečné hloubky a paty drž na podložce.",
        HERO,
    ),
    (
        "KROK 3",
        "NÁVRAT DO STOJE",
        "S výdechem zatlač chodidla do podložky a plynule narovnej kolena. Pánev drž neutrální a trup stále vzpřímený.",
        START,
    ),
]

STEP_BREATH = "Nádech při klesání. Výdech při návratu do stoje."
STEP_WATCH = (
    "Kolena veď ve směru špiček a paty nech na podložce. Neklesej níž, "
    "než dovolí bezpečný rozsah."
)
...
def build_guide():
    ...
    draw.text((62, 49), "PLIÉ DŘEP", font=F["title"], fill=INK)
    draw.text((62, 94), "Hýždě a stehna", font=F["small_b"], fill=TEAL_D)
    _, desc_bottom = draw_wrapped(
        draw, (62, 115), "Posiluje hýždě a stehna. Učí vést kolena bezpečně ve směru špiček.",
        F["tiny"], MUTED, 650, 2
    )
    margins["GUIDE DESCRIPTION"] = 140 - desc_bottom
    x = pill(draw, (62, 148), "Hýždě a stehna", F["small_b"])
    ...
    xs = [34, 274, 514]
    labels = [
        ("START", "Široký stoj", START),
        ("PLIÉ DŘEP", "Kolena ke špičkám", HERO),
        ("NÁVRAT", "Vzpřímený stoj", START),
    ]
    ...
    info = [
        ("breath", "DECH", "Nádech dolů. Výdech při návratu."),
        ("focus", "ZAMĚŘ SE", "Vzpřímený trup a stabilní pánev."),
...
def build_step():
    ...
    draw.text((62, 98), "PLIÉ DŘEP", font=F["small_b"], fill=TEAL_D)
