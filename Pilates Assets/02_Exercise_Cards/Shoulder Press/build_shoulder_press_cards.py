...
GUIDE_HOW = [
    ("1", "Postav se vzpřímeně. Činky drž u ramen, lokty pokrčené a střed těla zpevněný."),
    ("2", "S výdechem vytlač činky kontrolovaně vzhůru. Zápěstí drž rovně a ramena daleko od uší."),
    ("3", "S nádechem vrať činky pomalu k ramenům. Trup drž po celou dobu stabilní."),
]

GUIDE_WATCH = (
    "Neprohýbej bedra, nezvedej ramena k uším a nešvihej činkami. "
    "Zápěstí drž rovně."
)
...
STEP_TEXTS = [
    (
        "KROK 1",
        "VÝCHOZÍ POZICE",
        "Postav se vzpřímeně. Činky drž u ramen, lokty pokrčené, zápěstí rovná a střed těla zpevněný.",
        START,
    ),
    (
        "KROK 2",
        "TLAK NAD HLAVU",
        "S výdechem vytlač činky kontrolovaně vzhůru nad hlavu. Ramena drž daleko od uší a trup stabilní.",
        HERO,
    ),
    (
        "KROK 3",
        "KONTROLOVANÝ NÁVRAT",
        "S nádechem vrať činky pomalu k ramenům. Neprohýbej bedra a pohyb dokonči bez švihu.",
        START,
    ),
]

STEP_BREATH = "Výdech při tlaku vzhůru. Nádech při kontrolovaném návratu."
STEP_WATCH = (
    "Ramena drž daleko od uší, trup vzpřímený a bedra neutrální. "
    "Zápěstí drž rovně a nešvihej činkami."
)
...
def contain_image(path, size, margin=10):
    with Image.open(path) as source:
...
def build_guide():
    ...
    draw.text((62, 49), "TLAKY S ČINKAMI NAD HLAVU", font=F["title"], fill=INK)
    draw.text((62, 94), "Ramena • paže • stabilita trupu", font=F["small_b"], fill=TEAL_D)
    _, desc_bottom = draw_wrapped(
        draw, (62, 115), "Posiluje ramena a učí kontrolovaný tlak bez prohnutí v bedrech.",
        F["tiny"], MUTED, 650, 2
    )
    margins["GUIDE DESCRIPTION"] = 140 - desc_bottom
    x = pill(draw, (62, 148), "Ramena", F["small_b"])
    pill(draw, (x, 148), "Činky", F["small_b"])

    rounded(draw, (34, 196, 746, 653))
    paste_round(image, contain_image(HERO, (680, 393), 10), (50, 218, 730, 611))

    mini_y, mini_w, mini_h = 675, 218, 146
    xs = [34, 274, 514]
    labels = [
        ("START", "Činky u ramen", START),
        ("TLAK VZHŮRU", "Činky nad hlavu", HERO),
        ("NÁVRAT", "Činky zpět k ramenům", START),
    ]
    for index, (x0, (label, caption, source)) in enumerate(zip(xs, labels), 1):
        ...
        photo = contain_image(source, (mini_w - 22, mini_h), 6) if source == HERO else fit_image(source, (mini_w - 22, mini_h))
        paste_round(image, photo,
    ...
    info = [
        ("breath", "DECH", "Výdech při tlaku vzhůru. Nádech při návratu."),
        ("focus", "ZAMĚŘ SE", "Střed těla zpevni a zápěstí drž rovně."),
...
def build_step():
    ...
    draw.text((62, 98), "TLAKY S ČINKAMI NAD HLAVU", font=F["small_b"], fill=TEAL_D)
    ...
    for step_label, heading, body, source in STEP_TEXTS:
        ...
        image_bottom = y + 466
        photo = contain_image(source, (656, 352), 8) if source == HERO else fit_image(source, (656, 352))
        paste_round(image, photo, (62, y + 114, 718, image_bottom))
